...
__webpack_require__(/*! ./infinite */ "./resources/js/infinite.js");
...
window.infinite = _defineProperty({
  ...
  print_items: function print_items() {
    infinite.data.forEach(function (i, j) {
      var item_template = infinite.template;
  ...
  short_url: function short_url() {
    ...
    if (shorter.validate_url(long_url)) {
      axios.post('/url', {
        ...
        document.getElementById('short_url').value = response.data.shot_url;
      })["catch"](function (error) {
        console.log(error);
      });
    } else {}
